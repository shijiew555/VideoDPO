...
def clean_dir(dirname):
    ...
    for root, dirnames, filenames in os.walk(dirname):
        if "checkpoints" in dirnames:
            ...
            trainstep_checkpoints_path = os.path.join(checkpoints_path, "trainstep_checkpoints")
            
            if (len(os.listdir(checkpoints_path)) == 1 and 
                os.path.isdir(trainstep_checkpoints_path) and 
                len(os.listdir(trainstep_checkpoints_path)) == 0):
                # Remove the root directory
                shutil.rmtree(root)
            else:
                # Print the root directory path
                print(root,os.listdir(checkpoints_path),os.listdir(trainstep_checkpoints_path))
        # else:
