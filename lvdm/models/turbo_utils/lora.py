...
def inject_trainable_lora_extended(
    ...
    for _module, name, _child_module in _find_modules(
        ...
        if loras != None:
            param = loras.pop(0)
            if _module._modules[name].lora_up.weight.shape!=param.shape:
                print(name,_module._modules[name].lora_up.weight.shape,param.shape)
            # print(name,_module._modules[name].lora_up.weight.shape,param.shape)
            ...
            param = loras.pop(0)
            if _module._modules[name].lora_down.weight.shape!=param.shape:
                print(name,_module._modules[name].lora_down.weight.shape,param.shape)
            # print(name,_module._modules[name].lora_down.weight.shape,param.shape)
            if isinstance(param, torch.FloatTensor):
