...
if XFORMERS_IS_AVAILBLE is True:
    import math 
    def robust_memory_efficient_attention(q,k,v):
        if q.shape[0]>=65535:
            split_out= []
            split_numbers =math.ceil(q.shape[0]/65535)
            for split_i in range(split_numbers):
               if split_i!=split_numbers-1:
                    split_out.append(xformers.ops.memory_efficient_attention(q[split_i*65535:(split_i+1)*65535,...], k[split_i*65535:(split_i+1)*65535,...], v[split_i*65535:(split_i+1)*65535,...], attn_bias=None, op=None))
               else:
                    split_out.append(xformers.ops.memory_efficient_attention(q[split_i*65535:,...], k[split_i*65535:,...], v[split_i*65535:,...], attn_bias=None, op=None))
            out = torch.concat(split_out)
        else:
...
class CrossAttention(nn.Module):
    ...
    def efficient_forward(self, x, context=None, mask=None):
        ...
        # out = xformers.ops.memory_efficient_attention(q, k, v, attn_bias=None, op=None)
        out = robust_memory_efficient_attention(q,k,v)
        ## considering image token additionally
        if context is not None and self.img_cross_attention:
            k_ip, v_ip = map(
                lambda t: t.unsqueeze(3)
            ...
            out_ip = robust_memory_efficient_attention(q,k,v)
            out_ip = (
                out_ip.unsqueeze(0)
