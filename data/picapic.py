...
class PicAPic(Dataset):
    def __init__(self,
        ...
        not_split_idx = [i for i,label_0 in enumerate(dataset[self.subset_split]['label_0'])
                            if label_0 in (0,1) ]
        ...
        if dreamlike_pairs_only:
            orig_len = dataset[self.subset_split].num_rows
            dream_like_idx = [i for i,(m0,m1) in enumerate(zip(dataset[self.subset_split]['model_0'],
                                                                dataset[self.subset_split]['model_1']))
                                if ( ('dream' in m0) and ('dream' in m1) )]
            dataset[self.subset_split] = dataset[self.subset_split].select(dream_like_idx)
            new_len = dataset[self.subset_split].num_rows
            print(f"Eliminated {orig_len - new_len}/{orig_len} non-dreamlike gens for Pick-a-pic")
            
        if max_train_samples is not None:
            dataset[self.subset_split] = dataset[self.subset_split].shuffle(seed=seed).select(range(max_train_samples))
        # Set the training transforms
        print("Ignoring image_column variable, reading from jpg_0 and jpg_1")
        self.train_dataset = dataset[self.subset_split]#.with_transform(preprocess_train)
        # Preprocessing the datasets.
