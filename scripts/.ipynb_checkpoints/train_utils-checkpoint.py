...
def load_checkpoints(model, model_cfg):
    ...
    if check_config_attribute(model_cfg, "ref_model_checkpoint"):
        ...
        ref_model_ckpt = torch.load(ref_model_checkpoint, map_location="cpu")['state_dict']
    ...
    if check_config_attribute(model_cfg, "adapter_only"):
        pretrained_ckpt = model_cfg.pretrained_checkpoint
        assert os.path.exists(pretrained_ckpt), "Error: Pre-trained checkpoint NOT found at:%s"%pretrained_ckpt
        mainlogger.info(f">>> Load weights from pretrained checkpoint (training adapter only) {pretrained_ckpt}")
        # print(f"Loading model from {pretrained_ckpt}")
...
def get_empty_params_comparedwith_sd(model, model_cfg):
    ...
    is_load_refmodel = True if "ref_model_checkpoint" in model_cfg else False # added for dpo 
