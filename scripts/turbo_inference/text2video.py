...
# add lvdm path 
import sys 
sys.path.append("/root/DPO-videocrafter")
...
def generate_videos_from_prompts(prompts_file, output_dir, unet_dir, base_model_dir):
    ...
    for idx, prompt in tqdm(enumerate(prompts),total=len(prompts)):
        ...
        # print(result.shape)
        results_array = [result[i] for i in range(result.shape[0])]
...
# python text2video.py  --unet_dir ./unet_lora.pt  --base_model_dir ~/liurt_data/haoyu/DPO-videocrafter/init_ckpt/vc2/model.ckpt --prompts_file prompts/vidpro10k.txt  --output_dir vidpro10k-4

# python scripts/turbo_inference/text2video.py --unet_dir /root/autodl-tmp/results/baselines/20241031005638_vc2_turbo_dpo/checkpoints/last.ckpt --base_model_dir /root/autodl-tmp/model.ckpt --prompts_file prompts/test_prompts.txt --output_dir test_output_turbo
